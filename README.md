...
| **Closed Rolling Window** | **Redis List**<br>`kline:{SYMBOL}:1m` | List (fixed length of 200 bars)<br>10-element compact JSON array (no keys) | **Ultra-fast feature-computation cache for strategies.** 1m only, the most recent 200 closed bars. For coarser intervals, query the ClickHouse rollup tables directly. |
