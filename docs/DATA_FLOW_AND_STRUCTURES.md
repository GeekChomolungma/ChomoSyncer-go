...
- **Concurrency & Goroutine Model**:
  ...
  - **Goroutine Partitioning & Scheduling**:
    ...
    - **Keep-alive and disconnect self-healing**:
      - The official connector handles server-side pings by automatically replying with pongs, and re-creates the connection every 23h. That rebuild is break-before-make and **silent**: the connector logs a normal close and never reports an error, so `Errors()` stays quiet while frames sent in between are lost. The adapter ([`watchStatus`](../internal/collector/silentreconnect.go)) polls the connection status every 100ms and turns an `OPEN → CLOSING → CONNECTING → OPEN` cycle into a gap (`ws_silent_reconnects_total`), delivered through the same `OnGap` path.
...
- **Three trigger sources**:
  ...
  2b. **Silent connector reconnect (the 23h keep-alive)**: reported through the same `OnGap` → `HandleGap` path as above (see the collector section). Because resume-mode backfill only fills the *tail*, this trigger has to fire promptly.
  2c. **Integrity sweep (the safety net)**: [`Sweeper`](../internal/backfill/sweep.go) runs every `sweep_every` (30m). It asks ClickHouse ([`CHStore.FindGaps`](../internal/backfill/clickhouse.go)) for holes *between* existing 1m bars in the last `sweep_window` (24h), and submits them as a **repair-mode** request (`Request.Ranges`: exactly those ranges, not "from the last bar on"). Minutes Binance has no kline for (no trades) are remembered for `sweep_empty_ttl` so they are not re-requested every pass. Metrics: `backfill_sweep_*`. For holes older than the window use `cmd/test-tools/backfill_missing_1m.py`.
...
- **Start-up is a full pass.** On every start each symbol is asked for one whole page (~41 h) of the newest labels and everything returned is written; a longer gap is paged back to the last calibrated bar. Only the hourly passes skip symbols that are already current — see 10.3.
...
- **Concurrency & Goroutine Model**:
  - **Live loop** ([`Live.Run`](../internal/openinterest/live.go#L111)): one scheduler goroutine. For each boundary `B` it waits until `B − live_lead` (30s) and runs [`Cycle`](../internal/openinterest/live.go#L132): 8 workers pull symbols from a channel, paced at `fapi_rps` (25/s) and admitted by the weight gate. The round is abandoned at `B + live_accept_window` (60s). Every row of the round belongs to the bar that closes at `B` and stores the response as-is: the response's `time` is neither used to pick the bar nor to reject the value, it is kept in `snap_time` (for illiquid symbols it can be much older than the request).
  - **Hist reconciler** ([`Reconciler.Run`](../internal/openinterest/hist.go#L164)): one scheduler goroutine + 4 workers. [`LoadState`](../internal/openinterest/hist.go#L128) reads from ClickHouse, per symbol, the newest row with `src_rank >= 2` (retrying until the database answers). `plan` then decides per symbol. **Start-up (full) pass**: nothing is skipped and the window is widened to at least one whole page (`hist_max_limit − hist_limit_margin` bars back from now); a longer gap is refetched from the last calibrated bar (one bar of overlap); no calibrated row → the cold-start window (48h). **Hourly (incremental) pass**: already current → **skip, no request**; behind → refetch from that bar. Never further back than `hist_max_backfill` (7 days). Requests are sized to the gap (`limit = gap + margin`) and, because `startTime` alone cannot page forward, long gaps are paged **backwards** by moving `endTime` ([`fetchSymbol`](../internal/openinterest/hist.go#L375)). The start-up pass runs unspread; the hourly pass starts at `hh:05` and spreads its requests over 20 minutes, biggest gaps first, failed symbols retried once. The in-memory "calibrated up to" state advances only after the rows were flushed ([`Pass`](../internal/openinterest/hist.go#L262)).
...
- **Code Entry Points**:
  ...
  - Alignment rules: `histBarStart` in [`align.go`](../internal/openinterest/align.go); live rows use the round's boundary minus one bar (`Cycle`)
