...
- **Live** (`live.go`): every 5-minute boundary `B`, from `B − live_lead` (30s), snapshot every symbol with `GET /fapi/v1/openInterest`; the row is always the bar that closes at `B` and the response is stored as-is (its own `time` goes into `snap_time` for later audit; an illiquid symbol's older snapshot is simply that bar's value until hist calibrates it). `live_accept_window` only bounds how long after `B` a round may keep running. When the service starts less than `live_catchup_window` after a boundary, `Run` first snapshots the bar that boundary closed (hist has not published it yet). Requests go through the shared weight gate as `ClassLive`.
- **Hist** (`hist.go`): `GET /futures/data/openInterestHist`, label `T` is stored at `T − 5m`. The start-up pass is full: every symbol is asked for at least one whole page of the newest labels (500 bars ≈ 41 h) and everything returned is written, paging back through `endTime` (because `startTime` alone cannot page forward) to the newest calibrated row from ClickHouse (`maxIf(start_time, src_rank >= 2)`) if the gap is longer, never beyond `hist_max_backfill`. Then an incremental pass runs hourly at `hh:05`, spread over 20 minutes: per symbol it skips if current, otherwise fetches from the last calibrated bar, calibrating live rows. State is advanced only after the rows are flushed.
