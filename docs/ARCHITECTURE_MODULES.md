...
- **Live** (`live.go`): every 5-minute boundary `B`, from `B − live_lead` (30s), snapshot every symbol with `GET /fapi/v1/openInterest`; the row is always the bar that closes at `B` and the response is stored as-is (its own `time` goes into `snap_time` for later audit; an illiquid symbol's older snapshot is simply that bar's value until hist calibrates it). `live_accept_window` only bounds how long after `B` a round may keep running. Requests go through the shared weight gate as `ClassLive`.
