> **Language:** [English](DATA_FLOW_AND_STRUCTURES.md) | 简体中文
...
- **启动那一轮是全量的。** 每次启动，每个标的都请求一整页（约 41 小时）最新标签并写入所有返回内容；缺口更长就往回翻页到最新已校准的那根。只有每小时的校准才会跳过已是最新的标的——见 10.3。
