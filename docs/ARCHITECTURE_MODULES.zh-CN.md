> **Language:** [English](ARCHITECTURE_MODULES.md) | 简体中文
...
- **双重保活与看门狗 (`shard.go`)**：
  - 底层驱动内置自动 Ping/Pong 与 23 小时主动重建——但它**并不平滑**（先断后连，旧连接关闭到新连接开始读取之间的帧会丢），而且是**静默**的（不会向上层报任何错误）；因此适配层会监视连接状态并把它当作缺口上报，定时回查是最后的兜底（见回补一节）；
