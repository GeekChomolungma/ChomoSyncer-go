#!/usr/bin/env bash
# Fold the 5m open-interest history (market.fapi_oi_5m) into the rollup tables
# (15m/1h/4h/1d), one calendar month at a time, by running
# deploy/clickhouse/006_oi_rollup_backfill.sql per month. Idempotent (everything is recomputed
# from `FINAL`, newest rollup_version wins); run it AFTER 005_oi_rollups.sql. Months must be
# whole: 006 needs month-aligned ranges. The kline counterpart is rollup_kline_per_month.sh.
#
#   ./rollup_oi_per_month.sh --password '<pw>'
#   ./rollup_oi_per_month.sh --password '<pw>' --from 2026-08-01 --to 2026-10-01
#   CLICKHOUSE_PASSWORD='<pw>' ./rollup_oi_per_month.sh --dry-run
set -u
...
FROM=2000-08-01                          # Binance's OI archive starts in 2020-09 (first bar 2020-08-31 23:55)
TO=$(date -u -d "$(date -u +%Y-%m-01) +1 month" +%F)   # the next month start: includes the current month
DRY_RUN=0
SQL=deploy/clickhouse/006_oi_rollup_backfill.sql
