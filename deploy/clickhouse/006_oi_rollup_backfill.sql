-- ============================================================================
-- One-time historical fold: populate fapi_oi_{15m,1h,4h,1d,1mo} from the 5m history
...
--   every interval here, including 1mo):
...
--   IMPORTANT: m_start / m_end must be month boundaries. A range that cuts a bucket
--   in half writes a partial bucket (see the LOOKBACK note in 005).
-- ============================================================================
...
-- 1mo
INSERT INTO market.fapi_oi_1mo
    (symbol, start_time, samples,
     sum_open_interest_close, sum_open_interest_high, sum_open_interest_low,
     rollup_version)
SELECT
    symbol,
    toDateTime64(toStartOfMonth(start_time), 3, 'UTC') AS start_time_bucket,
    toUInt16(count()),
    argMax(sum_open_interest, start_time),
    max(sum_open_interest),
    min(sum_open_interest),
